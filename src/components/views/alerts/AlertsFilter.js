...
export default function AlertsFilter({
  ...
  return (
    <div className='AlertsFilter flex flex-col items-end space-y-3 sm:flex-row sm:items-center sm:space-x-6 sm:space-y-0'>
      {dirty && (
        <button
          className='border-1 hidden bg-white text-sm font-bold text-gray-400 underline sm:block sm:rounded-full dark:bg-gray-700'
      ...
      {dirty && (
        <button
          className='border-1 block rounded-full bg-white text-sm font-bold text-gray-400 underline sm:hidden dark:bg-gray-700'
          onClick={resetFilters}
        >
          Reset Filters
        </button>
